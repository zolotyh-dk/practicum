package sprint6.sorted_letters;
...
public class Practicum {
    ...
    private static void printOrderedByDateReceived(Set<Letter> letters) {
        ...
        Comparator<Letter> byDateReceived = Comparator.comparing(letter -> letter.dateReceived);
        Set<Letter> sortedByDateReceived = new TreeSet<>(byDateReceived);
        sortedByDateReceived.addAll(letters);

        for (Letter letter : sortedByDateReceived) {
            System.out.println("    * Письмо от " + letter.authorName + " поступило " + letter.dateReceived);
        }
    }
